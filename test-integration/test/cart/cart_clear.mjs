...
describe('Clear Cart', function () {
    it('should add multiple products to a users cart then clear the cart', async function () {
        ...
        let res_items_pre_delete = await axios.get(`${process.env.INF_API_ENDPOINT}main/cart/${user_id}`)

        let res_delete = await axios.delete(`${process.env.INF_API_ENDPOINT}main/cart/${user_id}`)
