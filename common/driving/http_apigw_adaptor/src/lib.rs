...
impl lambda_http::IntoResponse for HttpPortResponse<String> {
    fn into_response(self) -> Pin<Box<(dyn Future<Output = lambda_http::Response<lambda_http::Body>> + Send + 'static)>> {
        let (parts, body) = self.0.into_parts();
        let body = lambda_http::Body::from(body);
        Box::pin(async move {
            lambda_http::Response::from_parts(parts, body)
        })
    }
}
...
macro_rules! http_lambda_driving_adaptor {
    ($x:ident) => ( 
        async fn http_lambda_driving_adaptor (event: lambda_http::Request ) -> Result<lambda_http::Response<lambda_http::Body>, Error> {
            let blank_query_map = QueryMap::default();
            let path_parameters_ref = event.path_parameters_ref().unwrap_or(&blank_query_map);
            let query_string_parameters_ref = event.query_string_parameters_ref().unwrap_or(&blank_query_map);
            ...
            let generic_http_response = $x(path_parameters_ref, query_string_parameters_ref, &payload).await.unwrap();
            let lambda_http_response = HttpPortResponse(generic_http_response).into_response().await;
